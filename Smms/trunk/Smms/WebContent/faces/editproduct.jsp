...
<sj:div id="div4">
	<table id="table"
		...
		<tr>
			<td><sj:div id="div0">
				<sj:div id="div2">
					<s:url id="remoteurl" action="jsoneditproduct-admin" />
					<!-- EditProduct -->
					<s:url id="edit1url" action="add-sku-grid-admin" />
					<!-- AddSkuAct    (insert,update,del) -->
					<sjg:grid id="gridedittable" caption="ผลิตภัณฑ์" dataType="json"
						href="%{remoteurl}" pager="true" navigator="true" width="930"
						navigatorEdit="true" navigatorSearch="false"
						...
						<sjg:gridColumn name="name" index="name" title="ชื่อ ผลิตภัณฑ์"
							editrules="{required:true}" editable="true" width="200" />
						<sjg:gridColumn name="description" title="รายละเอียด ผลิตภัณฑ์"
							editrules="{required:true,minlength:5,required:true}"
							edittype="textarea" editable="true" width="250" search="false" />
						<sjg:gridColumn name="price" title="ราคากลางต่อหน่วย"
							formatter="currency" required="true"
							editrules="{number:true,required:true}"
							formatoptions="{decimalSeparator:'.',thousandsSeparator: ',', decimalPlaces: 2}"
							align="right" editable="true" width="180" search="false" />
						<sjg:gridColumn name="memberPrice" title="ราคาสมาชิกต่อหน่วย"
							formatter="currency" editrules="{number:true,required:true}"
							formatoptions="{decimalSeparator:'.',thousandsSeparator: ',', decimalPlaces: 2}"
							align="right" editable="true" width="180" search="false" />
						<sjg:gridColumn name="discount" title="ส่วนลดพิเศษ%"
							formatter="integer" editrules="{integer:true}" align="right"
							editable="true" search="false" />
						<sjg:gridColumn name="priceDiscount" formatter="currency"
							editrules="{number:true,required:true}"
							formatoptions="{decimalSeparator:'.',thousandsSeparator: ',', decimalPlaces: 2}"
							align="right" title="ราคาต่อหน่วยหลังหักส่วนลด" width="250"
							search="false" />
						<sjg:gridColumn name="sv" index="sv" title="Smile Value"
							editrules="{integer:true,required:true}"
							formatoptions="{thousandsSeparator: ','}" align="right"
							formatter="integer" sortable="false" editable="true" width="160"
							search="false" />
					</sjg:grid>
					<br />
